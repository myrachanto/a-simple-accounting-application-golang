...
func (paymentRepo paymentrepo) AllSearchSupplier(code,dated,searchq2,searchq3 string) (results []model.Payment, r *httperors.HttpError) {
	...
	if dated != "custom"{ 
		if dated == "In the last 24hrs"{
			d := now.AddDate(0, 0, -1)
			GormDB.Where("updated_at > ? AND itemcode = ?", d,code).Find(&results)
		}
		if dated == "In the last 7days"{
			d := now.AddDate(0, 0, -7)
			GormDB.Where("updated_at > ? AND itemcode = ?",d,code).Find(&results)
	...
	if dated == "custom"{
		...
		GormDB.Where("itemcode = ? AND updated_at BETWEEN ? AND ?",code,start, end).Find(&results)
	}
	IndexRepo.DbClose(GormDB)
	return results, nil
